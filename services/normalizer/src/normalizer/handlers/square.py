"""Handler for Square webhook events."""
...
from ..transformers.square import SquareTransformer
...
class SquareHandler:
    ...
    def process(
        ...
        """
        Process a raw Square webhook message.
        ...
        try:
            ...
            logger.warning("Failed to parse Square message: %s", e)
        ...
        if validation_result.is_valid and unified_event:
            logger.debug("Successfully processed Square event: %s", event_id)
            ...
            logger.warning(
                "Validation failed for Square event %s: %s",
                event_id,
    ...
    def _create_dlq_result(
        ...
        dlq_payload = {
            "event_id": f"square:{event_id}" if event_id else None,
            "provider": "square",
