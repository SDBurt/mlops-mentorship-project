...
2. Re-run initialization: `make init-polaris` (idempotent)
3. Check credentials in `set_pyiceberg_env.sh` match the generated credentials
...
**Solution**: Namespaces can be created later via Trino or using the dagster_user token after full RBAC setup:
