...
# MinIO
helm show values minio/minio > infrastructure/kubernetes/minio/values-default.yaml
```
...
- ✅ **MinIO** - Configured with standalone deployment
- ✅ **Airbyte** - Using external PostgreSQL, MinIO for S3 storage
- ✅ **Dagster** - Configured with user code deployment and MinIO integration
- ✅ **Trino** - Configured with Iceberg connector for MinIO S3

### Important TODOs

**MinIO:**
- [ ] Change default credentials for production
- [ ] Configure backup/replication strategy for production

**Airbyte:**
- [ ] Change MinIO password for production
- [ ] Configure Airbyte to use MinIO for state storage

**Dagster:**
- [ ] Update user deployment image after running `dagster init`
- [ ] Change PostgreSQL password for production
- [ ] Enable S3ComputeLogManager to store logs in MinIO

**Trino:**
- [ ] Configure S3 credentials for MinIO (use Kubernetes Secret)
- [ ] Phase 3: Configure Apache Polaris REST catalog
- [ ] Phase 3: Enable authentication for Trino web UI

### Lakehouse Integration

All services use MinIO as the central S3-compatible storage:

```
Airbyte → MinIO/S3 (raw data, Parquet)
           ↓
        Iceberg Tables
           ↓
Dagster ← MinIO/S3 (logs, artifacts)
Trino   ← MinIO/S3 (query data)
```
...
- **MinIO** - Root credentials configured in deployment manifest
