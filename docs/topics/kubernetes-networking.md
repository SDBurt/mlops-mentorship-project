...
In this lakehouse platform, networking is critical for service integration: [Airbyte](airbyte.md) writes to [MinIO](minio.md) S3, [Trino](trino.md) queries MinIO, [DBT](dbt.md) connects to Trino, and [Dagster](dagster.md) orchestrates everything. All communication happens via Kubernetes DNS without hardcoded IP addresses.

## Why Networking Matters for This Platform

**Service Discovery**: Components discover each other automatically using DNS names (e.g., `minio`) instead of IP addresses.

**Cross-Namespace Communication**: Services in different namespaces communicate seamlessly (Airbyte in `airbyte` namespace talks to MinIO in `minio` namespace).
...
# minio-0   1/1     Running   10.244.0.5   docker-desktop
```
...
spec:
  type: ClusterIP
  selector:
    app.kubernetes.io/name: minio  # Select pods with this label
  ports:
  - name: s3-api
    port: 3900
    targetPort: 3900
...
- **selector**: Matches pods with label `app.kubernetes.io/name=minio`
...
# Expected output:
# NAME     ENDPOINTS                          AGE
# minio   10.244.0.5:3900,10.244.0.5:3902    5m
```
...
- From `minio` namespace: `minio` (short form) or `minio` (FQDN)
- From `airbyte` namespace: Must use `minio` (cross-namespace)
...
#
# Name:      minio
# Address 1: 10.96.100.50 minio
```

**What this shows**:
- DNS server: 10.96.0.10 (CoreDNS service)
- Resolved IP: 10.96.100.50 (MinIO service ClusterIP)

**Test HTTP connectivity**:
```bash
# From Airbyte pod, access MinIO S3 API
kubectl exec -n lakehouse <airbyte-pod> -- curl -I http://minio:3900
...
kubectl port-forward -n lakehouse pod/minio-0 3900:3900
```
...
**Example - MinIO Headless Service**:
```bash
...
nslookup minio-0.minio-headless.minio.svc.cluster.local
# Answer: 10.244.0.5
```

**Pattern**: StatefulSet creates pods `minio-0`, `minio-1`, `minio-2`
**DNS names**:
- `minio-0.minio-headless.minio.svc.cluster.local` → Pod 0
- `minio-1.minio-headless.minio.svc.cluster.local` → Pod 1
- `minio-2.minio-headless.minio.svc.cluster.local` → Pod 2

**Why this matters**: Distributed systems like MinIO need to address specific nodes (e.g., "connect to replica 0 for reads").

### 6. Network Policies

**What it is**: Firewall rules for pod-to-pod traffic. By default, all pods can reach all pods (no restrictions).

**Example - Restrict MinIO Access**:
```yaml
...
**Scenario**: [Airbyte](airbyte.md) (in `airbyte` namespace) writes data to [MinIO](minio.md) (in `minio` namespace)
...
**What happens**:
1. Airbyte pod resolves `minio` via DNS → `10.96.100.50`
2. Connects to ClusterIP `10.96.100.50:3900`
3. Kubernetes routes traffic to `minio-0` pod (10.244.0.5:3900)
...
kubectl exec -n lakehouse $AIRBYTE_POD -- curl -I http://minio:3900
...
### Pattern 3: Trino Queries MinIO S3

**Scenario**: [Trino](trino.md) queries Iceberg tables stored in [MinIO](minio.md) S3
...
**What happens**:
1. Trino coordinator pod resolves `minio` → `10.96.100.50`
2. Sends S3 API requests (ListObjects, GetObject, etc.)
3. MinIO returns Parquet files
...
**Symptom**: `nslookup: can't resolve 'minio'`
...
```bash
# View pod DNS config
kubectl exec -n lakehouse minio-0 -- cat /etc/resolv.conf

# Expected output:
# nameserver 10.96.0.10
# search minio.svc.cluster.local svc.cluster.local cluster.local
# options ndots:5
```

**Test DNS from pod**:
```bash
kubectl run -it --rm debug --image=busybox --restart=Never -- nslookup minio
```
...
kubectl get svc minio -n lakehouse -o jsonpath='{.spec.selector}'

# Output: {"app.kubernetes.io/name":"minio"}

# Check pod labels
kubectl get pods -n lakehouse --show-labels

# Verify pod has matching label "app.kubernetes.io/name=minio"
```
...
kubectl exec -n lakehouse <pod> -- nslookup minio
```
...
```bash
kubectl get networkpolicy -n minio
kubectl describe networkpolicy <policy-name> -n minio
```

**Common causes**:
- Typo in DNS name (forgot namespace: `minio` vs `minio`)
...
- **[MinIO](minio.md)**, **[Airbyte](airbyte.md)**, **[Trino](trino.md)**, **[Dagster](dagster.md)**: All rely on DNS-based service discovery
...
kubectl run -it --rm debug --image=busybox --restart=Never -n lakehouse -- wget -O- http://minio:3900
```
