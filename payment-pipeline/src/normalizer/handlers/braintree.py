"""Handler for Braintree webhook events."""
...
from ..transformers.braintree import BraintreeTransformer
...
class BraintreeHandler:
    """Handles Braintree webhook events."""

    def __init__(self):
        self.transformer = BraintreeTransformer()
    ...
    def process(
        ...
        """
        Process a raw Braintree webhook message.
        ...
        try:
            ...
            logger.warning("Failed to parse Braintree message: %s", e)
        ...
        if validation_result.is_valid and unified_event:
            logger.debug("Successfully processed Braintree event: %s", event_id)
            ...
            logger.warning(
                "Validation failed for Braintree event %s: %s",
                event_id,
    ...
    def _create_dlq_result(
        ...
        dlq_payload = {
            "event_id": f"braintree:{event_id}" if event_id else None,
            "provider": "braintree",
